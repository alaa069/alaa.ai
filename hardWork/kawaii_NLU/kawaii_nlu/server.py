...
from kawaii_nlu.data_router import DataRouter, InvalidModelError
from kawaii_nlu.version import __version__
...
class KawaiiNLU(object):
    """Class representing Kawaii NLU http server"""
    ...
    def hello(self, request):
        """Main Kawaii route to check if the server is online"""
        return "hello from Kawaii NLU: " + __version__
    ...
    def version(self, request):
        """Returns the Kawaii server's version"""
    ...
    @requires_auth
    @check_cors
    def kawaiiconfig(self, request):
        """Returns the in-memory configuration of the Kawaii server"""
...
if __name__ == '__main__':
    ...
    kawaii_nlu_config = KawaiiNLUConfig(cmdline_args.get("config"), os.environ, cmdline_args)
    kawaii = KawaiiNLU(kawaii_nlu_config)
    logger.info('Started http server on port %s' % kawaii_nlu_config['port'])
    kawaii.app.run('0.0.0.0', kawaii_nlu_config['port'])
