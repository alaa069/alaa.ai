...
from kawaii_nlu.converters import load_data
from kawaii_nlu.model import Interpreter
from kawaii_nlu.model import Trainer
...
if typing.TYPE_CHECKING:
    from kawaii_nlu.persistor import Persistor
...
def create_argparser():
    ...
    parser.add_argument('-c', '--config', required=True,
                        help="Kawaii NLU configuration file")
...
def create_persistor(config):
    # type: (KawaiiNLUConfig) -> Optional[Persistor]
...
def init():  # pragma: no cover
    # type: () -> KawaiiNLUConfig
    """Combines passed arguments to create kawaii NLU config."""

    parser = create_argparser()
    args = parser.parse_args()
    config = KawaiiNLUConfig(args.config, os.environ, vars(args))
    return config


def do_train_in_worker(config):
    # type: (KawaiiNLUConfig) -> Text
...
def do_train(config, component_builder=None):
    # type: (KawaiiNLUConfig, Optional[ComponentBuilder]) -> Tuple[Trainer, Interpreter, Text]
