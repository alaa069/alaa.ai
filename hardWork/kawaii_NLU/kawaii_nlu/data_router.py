...
from kawaii_nlu import utils
from kawaii_nlu.components import ComponentBuilder
from kawaii_nlu.config import KawaiiNLUConfig
from kawaii_nlu.model import Metadata, InvalidModelError, Interpreter
from kawaii_nlu.train import do_train_in_worker
...
class DataRouter(object):
    ...
    def __init__(self, config, component_builder):
        self._training_processes = config['max_training_processes'] if config[
            'max_training_processes'] > 0 else 1
        self.config = config
        self.responses = DataRouter._create_query_logger(
            config['response_log'])
    ...
    def _create_query_logger(response_log_dir):
        ...
        # Ensures different log files for different processes in multi worker
        # mode
        if response_log_dir:
            # We need to generate a unique file name, even in multiprocess
            # environments
            timestamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
            log_file_name = "kawaii_nlu_log-{}-{}.log".format(
                timestamp, os.getpid())
            response_logfile = os.path.join(response_log_dir, log_file_name)
            # Instantiate a standard python logger, which we are going to use
            # to log requests
            utils.create_dir_for_file(response_logfile)
            query_logger = Logger(observer=jsonFileLogObserver(io.open(response_logfile, 'a', encoding='utf8')),
                                  namespace='query-logger')
            # Prevents queries getting logged with parent logger --> might log
            # them to stdout
    ...
    def __create_model_store(self):
        # Fallback for users that specified the model path as a string and
        # hence only want a single default model.
        ...
        for alias, model_path in list(model_dict.items()):
            try:
                ...
                logger.exception(
                    "Failed to load model '{}'. Error: {}".format(model_path, e))
        if not model_store:
            meta = Metadata({"pipeline": ["intent_classifier_keyword"]}, "")
            interpreter = Interpreter.load(
                meta, self.config, self.component_builder)
    ...
    def load_model_from_cloud(model_dir, config):
        try:
            from kawaii_nlu.persistor import get_persistor
    ...
    def __create_emulator(self):
        """Sets which NLU webservice to emulate among those supported by Kawaii"""

        mode = self.config['emulate']
        if mode is None:
            from kawaii_nlu.emulators import NoEmulator
            return NoEmulator()
        elif mode.lower() == 'wit':
            from kawaii_nlu.emulators.wit import WitEmulator
            return WitEmulator()
        elif mode.lower() == 'luis':
            from kawaii_nlu.emulators.luis import LUISEmulator
            return LUISEmulator()
        elif mode.lower() == 'api':
            from kawaii_nlu.emulators.api import ApiEmulator
            return ApiEmulator()
        else:
    ...
    def parse(self, data):
        ...
        for alia in alias:
            if alia not in self.model_store:
                try:
                    self.model_store[alia] = self.__interpreter_for_model(
                        model_path=alia)
                except Exception as e:
                    raise InvalidModelError(
                        "No model found with alias '{}'. Error: {}".format(alia, e))

            model = self.model_store[alia]
            response = model.parse(data['text'], data.get('time', None))
            if self.responses:
                self.responses.info(user_input=response, model=alia)

            resultatModel.append(response)

        return self.format_response(resultatModel)
    ...
    def get_status(self):
        ...
        models = [model for model in models if os.path.isfile(
            os.path.join(model, "metadata.json"))]
        return {
    ...
    def start_train_process(self, data, config_values):
        f = tempfile.NamedTemporaryFile(
            "w+", suffix="_training_data.json", delete=False)
        f.write(data)
        f.close()
        ...
        for key, val in config_values.items():
            _config[key] = val
        _config["data"] = f.name
        train_config = KawaiiNLUConfig(cmdline_args=_config)
