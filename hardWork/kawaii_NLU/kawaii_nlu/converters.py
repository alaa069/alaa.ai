...
from kawaii_nlu.tokenizers import Tokenizer
from kawaii_nlu.training_data import TrainingData, Message
...
def load_markdown_data(filename):
    ...
    from kawaii_nlu.utils.md_to_json import MarkdownToJson
...
def validate_kawaii_nlu_data(data):
    # type: (Dict[Text, Any]) -> None
    """Validate kawaii training data format to ensure proper training. Raises exception on failure."""
    from jsonschema import validate
    from jsonschema import ValidationError

    try:
        validate(data, kawaii_nlu_data_schema())
    except ValidationError as e:
        e.message += \
            ". Failed to validate training data, make sure your data is valid. " + \
            "For more information about the format visit " + \
            "https://kawaii-nlu.readthedocs.io/en/latest/dataformat.html"
        raise e


def load_kawaii_data(filename):
    # type: (Text) -> TrainingData
    """Loads training data stored in the kawaii NLU data format."""

    with io.open(filename, encoding="utf-8-sig") as f:
        data = json.loads(f.read())
    validate_kawaii_nlu_data(data)

    common = data['kawaii_nlu_data'].get("common_examples", list())
    intent = data['kawaii_nlu_data'].get("intent_examples", list())
    entity = data['kawaii_nlu_data'].get("entity_examples", list())
    regex_features = data['kawaii_nlu_data'].get("regex_features", list())
    synonyms = data['kawaii_nlu_data'].get("entity_synonyms", list())
    ...
    for filename in files:
        with io.open(filename, encoding="utf-8-sig") as f:
            try:
                ...
                elif "kawaii_nlu_data" in file_data:
                    return KAWAII_FILE_FORMAT
...
def load_data(resource_name, fformat=None):
    ...
    elif fformat == KAWAII_FILE_FORMAT:
        return load_kawaii_data(files[0])
